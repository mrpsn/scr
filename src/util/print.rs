...
impl FilePrinter {
    ...
    fn format_size(&self, size: u64) -> String {
        Self::format_size_static(size, self.size_factor)
    }
    ...
    pub fn print_final(
        ...
        // Drop the terminal interface to return to standard stdout mode
        self.terminal = None;

        // Manual print of the final table to avoid TUI viewport limits
        println!();
        ...
        // Header
        if self.print_index {
            print!("{:<width$}", "#", width = idx_w);
        }
        print!("{ :>width$}  ", self.size_heading, width = size_w);
        print!("{:<width$}  ", "Created", width = date_w);
        print!("{:<width$}  ", "Modified", width = date_w);
        print!("{:<width$}  ", "Used", width = date_w);
        println!("Path");
        ...
        for disk in &disks {
            if let Some(mount) = disk.mount_point().to_str() {
                if abs_path.to_string_lossy().starts_with(mount) {
                    if mount.len() > best_len {
                        best_len = mount.len();
